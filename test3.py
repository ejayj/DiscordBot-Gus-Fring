...
def int_to_server(server_int: int): #returns name, exe, and lnk names. 
    ...
    if server_int==1:
        ...
        return program,program_exe,program_lnk,program_dir
    elif server_int==2:
        program="TITAN"
        program_exe="TITAN.exe"
        program_lnk="TITAN.lnk"
...
def is_running(server: int): 
    ...
    program, program_exe, program_lnk = int_to_server(server) #will throw error if not given int or valid range
...
def start_server(server: int):
    ...
    print(f"ATTEMPTING TO OPEN: {program_lnk}")
    ...
    #TODO:
    #how cani check for success message? check is running? or does subprocess return TRUE or success as in the console?
        
    success = is_running(server) #or from subprocess
    if success:
        ...
        print(f"FAILED TO OPEN: {program_lnk}")
        return False
...
def stop_server(server: int):
    ...
    program,program_exe,program_lnk=int_to_server(server)
    
    os.system(f"taskkill -f -im {program_exe}")
    #how to check upon success? what if  taskkill freezes?will bot freeze? should i make it a subprocess?
...
def open_program(program): #1 for palworld, 2 for modded mc, 3 for regular mc
    ...
    if not is_running(program_exe):#if program is already running, don't open it
        ...
        subprocess.Popen("TITAN.exe")
        print(f"Successfully opened {program}")
