...
#TODO: UTILIZING BOT
#add rules channel to rxn sets. if someone removes or adds ANY emoji, i want it to supply or remove a role
...
#eventually, i want to make a poll bot! and some funny commands
...
def run_discord_bot():
    ...
    #********************
    #ADD REACTION MESSAGE -----> DEPRECATED
    #********************
    @client.tree.command()
    @app_commands.describe(message="Enter Message Here", emoji="Enter Emoji Here")
    async def addreactionmessage(interaction: discord.Integration, message: str, emoji: str): 
        """Create a reaction message here!"""
        
        #i could get rid of this emoji for custom emojis....
        
        # #check if emoji was supplied
        # if emoji.decode('utf-8') in emoji.UNICODE_EMOJI: #or if unicode(i, "utf-8") in emoji.UNICODE_EMOJI
        #     pass #emoji was supplied
        # else:
        #     await interaction.response.send_message(f"You did not supply a valid emoji!!", ephemeral=True)
        ...
        #add message and channel to the function (maybe, you can only have one?)
        REACTION_MESSAGE.add(message.id)
        REACTION_CHANNEL_ID.add(interaction.channel)
        
        #run await save_to_json() function
        print(interaction.channel)
        
        #testing two things:
        #1: if custom emojis can be used and pased
        #2: if interacton.channel passes an id
    ...
    async def on_raw_reaction_add(reaction: discord.Reaction):
        ...
        if await check_reaction(reaction,user):
            #do something
            print('this reaction triggers something')
            
            #depending on what message id, it will trigger different events
            if reaction.message_id=='': #the rules channel
                #add role function
                pass
    ...
    async def on_raw_reaction_remove(reaction: discord.Reaction):
        ...
        if await check_reaction(reaction,user):
            #do something
            print('this reaction removal triggers something')

        #i can then do something, like remove a rank from said user
    ...
    @client.tree.command()  
    async def reactionrolestest(interaction: discord.Integration):
