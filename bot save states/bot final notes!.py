...
def run_discord_bot():
    ...
    async def basicresponse(interaction: discord.Integration):
        ...
        #this is for testing the reaction message idea
        REACTION_MESSAGE.add(await interaction.original_response())
    ...
    REACTION_MESSAGE = set() #a set of reaction messages that we want to look for
    #eventually i'll want to extract this data from a json save file
    
    #detects all reactions on the server in specified, reaction channel id
    #to have multiple reaction channels, or messages, i would add a set() of channel ids in the reaction_channel_id
    ...
    async def on_reaction_add(user, reaction: discord.Reaction):
        ...
        for message in REACTION_MESSAGE: #really we just need its id
            if message.id == reaction.message.id: #if a reaction message, continue
                break
    ...
    async def on_raw_reaction_remove(reaction: discord.Reaction):
        ...
        if channel != REACTION_CHANNEL_ID:
            print("wrong channel for removal id")
            return
        ...
        print(f'User {user} removed reaction {reaction_emoji} in channel')
